...
import { Button, Card, EmptyState, IconBtn, Pill, type PillTone, TopBar } from '@/src/ui/primitives/v2';
...
export default function NdtLedgerScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: tokens.bg }}>
      {/* NDT routes aren't centrally registered yet (Task 14), so suppress the
          native header here rather than relying on a layout. */}
      ...
      {/* CRITICAL (mirrors records.tsx): never wrap a FlatList in a ScrollView —
          it disables virtualization. The summary/disclaimer ride as the list
          header so the whole screen scrolls as one virtualized surface. */}
      {!inspections.data ? null : (
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <Reveal index={index} dedupeKey={item.id}>
              <NdtRow
                inspection={item}
                onPress={() => router.push(`/ndt/${item.id}` as never)}
              />
            </Reveal>
          )}
        />
...
function NdtSummaryHeader({ summary, selfLoggedHours, verifiedHours }: NdtSummaryHeaderProps) {
  const { tokens } = useTheme();
...
function NdtRow({
  ...
  return (
    <Card padding={14} interactive onPress={onPress} accessibilityLabel={`${inspection.method} record at ${inspection.site}`}>
