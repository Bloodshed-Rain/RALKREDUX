// RALKREDUX — Theme system
// 6 palettes. Every theme exports the same token shape.
// Tokens are CSS custom properties set on the prototype root so we can
// transition palettes smoothly without re-rendering React.
...
const THEME_ORDER = ['tungsten', 'mariner', 'verdigris', 'heliotype', 'sandstone', 'mercury'];

function applyTheme(root, themeKey) {
  const theme = THEMES[themeKey] || THEMES.tungsten;
...
const ThemeContext = React.createContext({ theme: 'tungsten', setTheme: () => {} });

function ThemeProvider({ children, initial = 'tungsten', target }) {
  const [theme, setTheme] = React.useState(initial);
  React.useEffect(() => {
    const root = target?.current || document.documentElement;
    applyTheme(root, theme);
  }, [theme, target]);
  return (
    <ThemeContext.Provider value={{ theme, setTheme, themes: THEMES, order: THEME_ORDER }}>
      {children}
...
Object.assign(window, { THEMES, THEME_ORDER, applyTheme, ThemeProvider, useTheme, ThemeContext });
